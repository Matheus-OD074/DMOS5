...
class MainActivity : AppCompatActivity(), OnClickListener {
    ...
    private fun calculate(){
        if(gasolineEditText.text.toString().isEmpty() || ethanolEditText.text.toString().isEmpty()){
            Toast.makeText(this,"Informe o valor dos dois combustíveis.", Toast.LENGTH_SHORT).show()
            mTextView.text = ""
        }else{
            ...
            val result = etha / gas
            if(result < 0.7){
                mTextView.text = getString(R.string.anwser_ethanol)
                mTextView.setTextColor(resources.getColor(R.color.ethanol,this.theme))
            }else{
                mTextView.text = getString(R.string.anwser_gas)
